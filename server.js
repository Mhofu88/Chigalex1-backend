...
console.log("✅ Routers loaded (missing ones skipped, no crash)");

// ════════════════════════════════════════════
// ── PI NODE SDK (optional) ──
// ════════════════════════════════════════════
let pi = null;
try {
  ...
  console.warn('⚠️ Pi SDK not loaded - pi-backend missing or env not set:', e.message);
}
...
app.post('/complete-payment', rateLimit(10, 60_000), async (req, res) => {
  ...
  const country = sanitizeString(req.body.country || '', 64);  const lang = sanitizeString(req.body.lang || 'en', 10);
...
// ════════════════════════════════════════════
try { require('./merchant-directory')(app, redis, rateLimit, sanitizeString, isValidUsername, validateAdminKey, trackEvent); console.log('✅ merchant-directory loaded'); } catch(e){ console.warn('⚠️ merchant-directory skipped:', e.message); }

// ════════════════════════════════════════════
// ── REFERRAL SYSTEM MODULE ──
// ════════════════════════════════════════════
try { require('./referral-system')(app, redis, rateLimit, sanitizeString, isValidUsername, validateAdminKey, trackEvent); console.log('✅ referral-system loaded'); } catch(e){ console.warn('⚠️ referral-system skipped:', e.message); }
...
app.post('/ambassador/approve', async (req, res) => {
  if (!requireRedis(res)) return;
  const { admin_username, pi_username } = req.body;  if (!ADMIN_ACCOUNTS.includes((admin_username || '').toLowerCase())) {
    return res.status(403).json({ error: 'Admin access required' });
  }
  try {
    ...
    const application = typeof appData === 'string' ? JSON.parse(appData) : appData;
    const { month } = getPeriodKey();
...
app.post('/ambassador/record-recruit', async (req, res) => {
  ...
  try {
    ...
    const amb = typeof ambData === 'string' ? JSON.parse(ambData) : ambData;
    ...
    try {
      ...
      if (existing) history = typeof existing === 'string' ? JSON.parse(existing) : existing;
    } catch(e) {}
...
app.get('/ambassador/profile/:username', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (histData) history = typeof histData === 'string' ? JSON.parse(histData) : histData;
    } catch(e) {}
...
app.get('/ambassador/dashboard/:username', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const notifParsed = notifData ? (typeof notifData === 'string' ? JSON.parse(notifData) : notifData) : null;
      if (notifParsed) notifications = notifParsed.filter(n => !n.read);
    } catch(e) {}
...
app.get('/ambassador/notifications/:username', async (req, res) => {
  ...
  try {
    const notifData = await redis.get(`notifications:${username}`);
    const notifs = notifData ? (typeof notifData === 'string' ? JSON.parse(notifData) : notifData) : [];
...
app.listen(PORT, () => {
  console.log(`🚀 Chigalex1 FULL LIVE on port ${PORT}`);
